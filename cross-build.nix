...
defaultPlatformProject.pkgs.lib.mapAttrs (_: pkgs: rec {
  ...
  hsprjup = import ./default.nix { nativePkgs = pkgs;
                                     customModules = [ { packages.hsprjup.dontStrip = false; } ] ++ (if pkgs.stdenv.hostPlatform.isMusl then 
                                                        [
                                                          ...
                                                          { packages.hsprjup.configureFlags = 
                                                            [
